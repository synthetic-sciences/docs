...
<Steps>
  <Step title="Run a unified search">
    ...
    - **Graph-as-KB**, committed Atlas nodes and their content, embedded into searchable chunks.
  ...
  <Step title="Start an Oracle job for deeper synthesis">
    ...
    Check the job status with `oracle_job_status`, or stream live updates with `oracle_job_stream`. In the **Oracle** tab, you can see job history and results without leaving the node view.
...
|------|-------|-------|-----------|
| `search` | Fast | Shallow | You know roughly what you're looking for |
| `oracle_start_job` (quick) | Medium | Medium | You want a synthesis in under a minute |
| `oracle_start_job` (deep / oracle) | Slow | Deep | You want a thorough literature synthesis |
| `tracer_search` | Medium | Code-focused | You need to find implementation patterns across repos |
