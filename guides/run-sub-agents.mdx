...
After you approve an experiment proposal, Atlas delegates execution to a sandboxed sub-agent that runs independently of your chat session. The sub-agent receives a compiled payload from durable Atlas state, not your chat history, and reports telemetry back as it works. This guide covers every step from approval to result.
...
<Steps>
  <Step title="Approve the proposal in the UI">
    ...
    <Note>
      If the agent needs to request approval programmatically rather than via the UI card, it calls `request_compute_grant_approval` to initiate the approval session and prompt you for confirmation.
    </Note>
  </Step>
  ...
  <Step title="The agent calls spawn_experiment_agent">
    ...
    2. Calls `compute_acquire` to lease a GPU from the selected provider.
  ...
  <Step title="The sub-agent executes">
    The sub-agent runs in an isolated sandbox. It receives the following compiled payload, sourced entirely from durable Atlas state, not from chat history:
...
Atlas supports two compute backends:
...
Browse available GPU SKUs with `compute_list_options` before approving a proposal if you want to select a specific instance type.

## Safety boundaries

Atlas is intentionally conservative with compute:
