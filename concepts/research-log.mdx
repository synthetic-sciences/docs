...
<CardGroup cols={2}>
  <Card title="Decisions" icon="check-to-slot">
    Explicit choices made during the research process — which method to use, which hypothesis to prioritize, which approach to abandon.
  </Card>
  <Card title="Hypotheses" icon="question">
    Formal research hypotheses entered into the record, distinct from the hypothesis field on an empirical node.
  </Card>
  <Card title="Dead ends" icon="ban">
    Approaches that were tried and ruled out. Dead ends are valuable: they prevent future agents and collaborators from repeating the same wrong turns.
  </Card>
  <Card title="Assumptions" icon="triangle-exclamation">
    Explicit assumptions underpinning an experiment or analysis — the conditions that must hold for a result to be valid.
  </Card>
  <Card title="Results" icon="chart-line">
    Outcomes of experiments or analyses, including quantitative metrics and qualitative assessments.
  </Card>
  <Card title="Failures" icon="circle-xmark">
    Recorded experiment failures, separate from dead ends — a failure means the experiment ran but the outcome was not what was expected.
  </Card>
  <Card title="Pivots" icon="rotate">
    Documented changes in research direction — when you decide to shift focus, the log captures why.
  </Card>
  <Card title="Compute events" icon="server">
    Proposal events (when the agent stages an experiment), approval events (when you approve compute spending), and agent telemetry milestones (progress updates from running sub-agents).
  </Card>
</CardGroup>
