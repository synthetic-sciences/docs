...
When compute is approved, Atlas creates two records:
...
You can view active leases and their status from the **Agents** tab in the agent UI, or by querying `compute_status` through the agent tools.
...
Use `compute_list_options` from the agent tools to get the current list of available SKUs with pricing details.

## Compute providers

Atlas currently supports two compute providers:
...
Create a campaign budget with `create_campaign_budget`. The agent can check the remaining budget at any point with `check_budget` and will stop proposing new compute when the campaign budget is exhausted.

<Tip>
  Set a campaign budget before starting an autonomous research run with `atlas-auto`. This ensures the agent advances the frontier up to a defined cost limit and stops automatically, rather than running until you intervene.
</Tip>
...
You can view active approval sessions with `list_approval_sessions` and manually expire a session with `expire_approval_session` if you need to terminate a run early.
