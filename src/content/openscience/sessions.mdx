...
| `-m, --model <provider/model>` | Model for this run, e.g. `anthropic/claude-opus-5`. See [Models](/openscience/models). |
...
| `--format <default\|json>` | `json` emits one JSON event per line (`tool_use`, `step_start`, `step_finish`, `text`, `error`) with the session id. Pipe it to a file and parse line by line. |
