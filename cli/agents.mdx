...
- `plan` is read-only. Same skills and context, but cannot mutate anything.
...
The agent is orthogonal to the [research mode](/cli/modes). You can be in `chemistry` mode with the `build` agent, or `ml` mode with the `plan` agent, or any other combination.
...
`plan` has the same skills behind it but cannot mutate anything. No file writes, no shell commands that touch state, no git operations that change refs. Use it when you want the agent to analyze code or design an approach without the risk of accidental changes.
...
- **[Research modes](/cli/modes).** Modes are orthogonal to agents. Pick one of each.
- **[Skills](/cli/skills).** Bundled playbooks the agent loads on demand within a mode.
