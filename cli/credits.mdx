...
Your Synci CLI subscription includes credits that cover the agent's work. Every call reports its cost back to your dashboard, the balance is checked before each request, and the CLI blocks new prompts when credits hit zero. You don't pick a provider, the credit envelope abstracts that away.

## How credits work

Each call consumes credits proportional to the input and output tokens. The cost is computed from the active research mode and the work the agent performed. You see the cost in the dashboard and in `synci stats`.
...
To restore service, top up credits in the dashboard, then run `synci connect status` (or restart the session) so the CLI picks up the new balance.
...
- **[Web UI](/cli/web).** Watch credit usage live in the recommended interface.
