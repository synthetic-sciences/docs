...
synsc web --model claude-opus-4
synsc web --host 0.0.0.0    # expose to LAN (use carefully)
```

Flags:

- `--port <n>` — port to bind on `127.0.0.1` (default: `4444`).
- `--host <addr>` — interface to bind on (default: `127.0.0.1`).
...
Start a TUI session in the build agent on the default model. Useful when you want everything in the terminal.
...
| `/agent <name>` | Switch agents (also via the agent toggle in web/desktop, or <kbd>Tab</kbd> in TUI). |
