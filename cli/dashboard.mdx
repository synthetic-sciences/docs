---
title: "The Synci CLI dashboard at cli.syntheticsciences.ai"
...
Updating a credential is instant, the next session start picks up the new value. To force an immediate refresh in a running session, run `synci connect status` again.

See [Credentials](/cli/credentials) for the in-CLI behavior.

## Sessions panel

Every time you run `synci`, a session record opens in the dashboard. While the session is active, the dashboard shows live model usage and cost. After the session ends, the record persists with:
